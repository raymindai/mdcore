...
| Embed (iframe) | ✅ |
| My Documents + 자동저장 | ✅ |
| 바이럴 뱃지 ("Published with mdfy.cc") | 🔜 |
| 유료화 (Pro 티어) | 안정화 후 결정 |

### 3.2 VS Code Extension

**VS Code 안에서 mdfy.cc와 동일한 렌더링 + 편집 + 클라우드 싱크.**

| 기능 | 상태 |
|------|------|
| comrak WASM 렌더링 (mdfy.cc 동일 엔진) | ✅ |
...
**ChatGPT/Claude/Gemini 출력을 원클릭 캡처 → mdfy.cc 문서화.**
...
mdcore README.md --share       # mdfy.cc 공유
```
...
```
┌─ 사용자 표면 (Surfaces) ──────────────────────────┐
│  mdfy.cc · VS Code · Chrome · Mac · QuickLook     │
│  Terminal · API                                    │
└───────────────────┬───────────────────────────────┘
                    │
┌───────────────────┴───────────────────────────────┐
...
### 런칭 (전 기능 무료)
1. 바이럴 뱃지 ("Published with mdfy.cc")
2. mdfy.cc + Chrome ext + VS Code ext 동시 출시
