  ...
  useEffect(() => {
    ...
    fetch("/api/bundles", { headers: authHeaders })
      ...
      .then(data => {
        ...
        // Refresh local bundle Tab.title from server. Without this, a bundle
        // renamed via another channel (or first opened when the title was
        // still the autogenerated draft) leaves the local tab pinned to its
        // original title, which is what the user sees in Recent / the
        // bundle viewer header even though MD Bundles in the sidebar shows
        // the current name (it reads from `bundles[]` directly).
              ...
              return (<>
                <div className="shrink-0 flex flex-col">
                  <div
                    ...
                    className="flex items-center gap-1.5 px-3 h-7 cursor-pointer select-none group/sec hover:bg-[var(--toggle-bg)]"
                    style={{ background: "color-mix(in srgb, var(--background) 25%, var(--surface) 75%)", borderTop: "1px solid var(--border)", borderBottom: "1px solid var(--border)", position: "sticky", top: 0, zIndex: 10 }}
