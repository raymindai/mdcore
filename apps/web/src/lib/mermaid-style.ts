/**
 * Mermaid SVG restyler — mdfy.cc
 * In-place DOM modification. No rebuild, no layout changes.
 * Only touches: fill, stroke, stroke-width, rx/ry, font, color.
 */
...
export function styleMermaidSvg(svgString: string, isDark: boolean): string {
  ...
  // ── Nodes: only style the LAST shape (visible one), hide others ──
  svg.querySelectorAll(".node").forEach((node) => {
    ...
    shapes.forEach((s, i) => {
      if (i < shapes.length - 1) {
        // Hide shadow/background shapes
        s.setAttribute("fill", "none");
        s.setAttribute("stroke", "none");
      } else {
        // Style the visible shape
        if (!accented) {
  ...
  // ── Edge paths ──
  svg.querySelectorAll(".edgePaths path, .flowchart-link, .edge-pattern-solid").forEach((el) => {
  ...
  svg.querySelectorAll(".task").forEach((el) => {
    ...
    if (/done/.test(cls)) {
      ...
      el.setAttribute("fill", isDark ? "rgba(251,146,60,0.2)" : "rgba(234,88,12,0.12)");
      el.setAttribute("stroke", p.accent);
    } else {
      el.setAttribute("fill", p.nodeFill);
      el.setAttribute("stroke", p.nodeStroke);
  ...
  svg.querySelectorAll(".er.attributeBoxOdd, .er.attributeBoxEven").forEach((el) => {
    el.setAttribute("fill", p.clusterFill);
    el.setAttribute("stroke", p.clusterStroke);
  });
  ...
  svg.querySelectorAll("text").forEach((el) => {
    ...
    const isDim = !!el.closest(".edgeLabel") ||
...
function isAccent(fill: string): boolean {
  ...
  return f.includes("fb923c") || f.includes("f97316") || f.includes("ea580c") ||
         f.includes("orange") || (f.startsWith("rgb") && f.includes("251") && f.includes("146"));
}
