...
expect("returns multiple passages",       out.length >= 2, `got ${out.length}`);
expect("most-dense passage first",        out[0].toLowerCase().includes("authored memory"));
expect("ignores code fences",             !out.some((p) => p.includes("const authored = true")));
expect("ignores frontmatter",             !out.some((p) => p.includes("title: Test")));
expect("skips paragraphs without mention", !out.some((p) => p.toLowerCase().includes("random unrelated paragraph")));
...
expect("clipped to maxChars",             capped[0].length <= 100, `got length ${capped[0]?.length}`);
expect("clipped passage ends with ellipsis", capped[0].endsWith("…"));
...
expect("returns only maxPassages",         limited.length === 3, `got ${limited.length}`);

// Edge: empty / null / unmatched.
expect("empty markdown → []",              extractPassagesFor("", "x", 3, 200).length === 0);
expect("empty needle → []",                extractPassagesFor("hello world", "  ", 3, 200).length === 0);
expect("no matches → []",                  extractPassagesFor("hello world", "xyz", 3, 200).length === 0);
...
expect("regex-special concept labels match", escaped.length >= 2, `got ${escaped.length}`);
